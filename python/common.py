    ...
    model = tf.keras.Sequential(
        [
            tf.keras.layers.Conv2D(
                32, (3, 3), activation="relu", input_shape=FEATURES_SHAPE
            ),
    ...
    class MNISTModule(tf.Module):
        ...
        @tf.function(
            input_signature=[
                ...
                tf.TensorSpec(
                    [batch_size, NUM_CLASSES], tf.float32
...
def test_comparison(
    self, test_images, test_labels, img_dir="", num_samples=10, use_mlir_approx=True
):
    """
    ...
    print(
        f"Full Test Set - Approximate Model Accuracy: {approx_correct / len(test_images):.4f}"
    )
...
def test_load(img_dir="", load_mlir_path=OUT_DIR + "output.mlir"):
    ...
    func_sub.test_comparison(x_test, y_test, img_dir, num_samples=10)
