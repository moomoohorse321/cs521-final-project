...
def test_comparison_modified(
    ...
    if (
        show_sample_plots
        and num_samples_to_plot > 0
        and num_samples_to_plot <= len(test_images)
    ):
        ...
        for i, idx in enumerate(indices):
            ...
            if use_mlir_approx:
                ...
                approx_result_host = self.approx_kernel.approx_predict(
                    test_image
                )  # .to_host() might be needed depending on type
                approx_result_np = (
                    approx_result_host.to_host()
                    if hasattr(approx_result_host, "to_host")
                    else approx_result_host
                )
            else:
                ...
                approx_result_np = (
                    approx_result_tf.numpy()
                    if hasattr(approx_result_tf, "numpy")
                    else approx_result_tf
                )

            approx_pred = np.argmax(approx_result_np)

            if exact_pred == true_label:
                correct_exact_sample += 1
            if approx_pred == true_label:
                correct_approx_sample += 1
            ...
            plt.title(
                f"Exact: {exact_pred}" + (" ✓" if exact_pred == true_label else " ✗")
            )

            plt.subplot(num_samples_to_plot, 3, i * 3 + 3)
            plt.bar(range(NUM_CLASSES), approx_result_np)
            plt.title(
                f"Approx: {approx_pred}" + (" ✓" if approx_pred == true_label else " ✗")
            )
        ...
        if num_samples_to_plot > 0:
            ...
            print(
                f"Exact Model Accuracy (Sampled): {correct_exact_sample / num_samples_to_plot:.2f}"
            )
            print(
                f"Approximate Model Accuracy (Sampled): {correct_approx_sample / num_samples_to_plot:.2f}"
            )
            print(
                f"Average KL Divergence (Sampled): {avg_kl_div_sample / num_samples_to_plot:.6f}"
            )
    ...
    for i in range(num_full_test_samples):
        ...
        if use_mlir_approx:
            ...
            approx_result_tf = self.approx_kernel.approx_predict(
                test_image
            )  # Ensure this method exists on your approx_kernel
            approx_result_np = (
                approx_result_tf.numpy()
                if hasattr(approx_result_tf, "numpy")
                else approx_result_tf
        ...
        if exact_pred == true_label:
            exact_correct_full += 1
        if approx_pred == true_label:
            approx_correct_full += 1

    print(
        f"\rProcessing full test set: {num_full_test_samples}/{num_full_test_samples}... Done."
        + " " * 10
    )

    exact_accuracy_full = (
        exact_correct_full / num_full_test_samples if num_full_test_samples > 0 else 0
    )
    approx_accuracy_full = (
        approx_correct_full / num_full_test_samples if num_full_test_samples > 0 else 0
    )
    ...
    if exact_total_time > 0 and num_full_test_samples > 0:  # Avoid division by zero
        ...
        print(
            f"Full Test Set - Avg Exact Time per sample: {avg_exact_time*1000:.2f} ms"
        )
        print(
            f"Full Test Set - Avg Approx Time per sample: {avg_approx_time*1000:.2f} ms"
        )
        performance_improvement = (
            (avg_exact_time - avg_approx_time) / avg_exact_time
        ) * 100
        print(
            f"Full Test Set - Performance Improvement (Approx vs Exact): {performance_improvement:.2f}%"
        )
...
def run_insight1_experiments():
    ...
    try:
        ...
        exact_module = train_exact_module(
            exact_module, (x_train, y_train, y_train_onehot), epochs=5
        )
    ...
    temp_approx_kernel_for_exact_eval = get_approx_kernel(
    ...
    ):  # If compile_exact exists and is needed
        temp_func_sub_for_exact.compile_exact()

    temp_func_sub_for_exact.test_comparison = test_comparison_modified.__get__(
        temp_func_sub_for_exact
    )
    ...
    print(
        f"Baseline Exact Model Accuracy (full test set): {exact_accuracy_baseline:.4f}"
    )
    if exact_accuracy_baseline == 0:
        print(
            "ERROR: Baseline exact accuracy is 0. Cannot calculate accuracy drop. Please check the exact model."
        )
        return
    ...
    for n_samples in num_samples_values:
        ...
        func_sub_exp1 = FuncSubstitute(
            exact_module, approx_kernel_exp1, FEATURES_SHAPE, BATCH_SIZE
        )
        ...
        func_sub_exp1.train_approx(
            use_provided=True,
            user_data=x_train[:n_samples],
            epochs=fixed_epochs_for_exp1,
        )
        ...
        if exact_accuracy_baseline == 0:
            print(
                "Error: exact_accuracy_baseline is zero, cannot compute accuracy drop. Skipping."
            )
            accuracy_drop = float("nan")  # Or handle as an error
        else:
            accuracy_drop = (
                (exact_accuracy_baseline - approx_acc) / exact_accuracy_baseline
            ) * 100

        accuracy_drops_exp1.append(accuracy_drop)
        print(
            f"  Approx Acc: {approx_acc:.4f}, Accuracy Drop: {accuracy_drop:.2f}% (vs baseline {exact_accuracy_baseline:.4f})"
        )
    ...
    ax1.grid(
        True, which="major", ls="--", axis="y", color=color_acc, alpha=0.7
    )  # Grid for primary y-axis
    ...
    ax2.legend(
        lines + lines2, labels + labels2, loc="center right"
    )  # Adjust location as needed, e.g., 'best', 'upper left'

    fig.tight_layout()  # Adjust layout to prevent labels from overlapping
    plt.savefig(IMG_DIR + "plot_accuracy_drop_and_training_time_vs_samples.png")
    plt.show()
    print(
        "Plot for Experiment 1 saved as  ",
        IMG_DIR,
        "plot_accuracy_drop_and_training_time_vs_samples.png",
    )
    ...
    if fixed_samples_for_exp2 > len(x_train):
        ...
        print(
            f"Adjusted fixed samples for Exp2 to {fixed_samples_for_exp2} (max available)."
        )
    ...
    for n_epochs in epochs_values:
        ...
        func_sub_exp2 = FuncSubstitute(
            exact_module, approx_kernel_exp2, FEATURES_SHAPE, BATCH_SIZE
        )
        if hasattr(func_sub_exp2, "compile_exact"):
            func_sub_exp2.compile_exact()

        func_sub_exp2.train_approx(
            use_provided=True,
            user_data=x_train[:fixed_samples_for_exp2],
            epochs=n_epochs,
        )
    ...
    print(
        "Plot for Experiment 2 saved as ", IMG_DIR, "plot_accuracy_drop_vs_epochs.png"
    )
