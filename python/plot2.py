...
from common import load_data, create_mnist_module, OUT_DIR
...
def post_train_model(model, data, epochs=5):
    ...
    for epoch in range(epochs):
        ...
        for step in range(steps_per_epoch):
            ...
            if step % 10 == 0:
                print(
                    f"\rEpoch {epoch+1}/{epochs}, Step {step}/{steps_per_epoch}", end=""
                )

        # Print epoch summary (overwrites previous line)
        print(
            f"\rEpoch {epoch+1}/{epochs} complete, Average Loss: {epoch_loss / steps_per_epoch}",
            end="",
        )
    print("\nTraining complete.")
...
def initial_train_model(model_module, data, epochs=1):
    ...
    for epoch in range(epochs):
        ...
        for step in range(steps_per_epoch):
            ...
            if step % 20 == 0:
                print(
                    f"\r Epoch {epoch+1}/{epochs}, Step {step}/{steps_per_epoch}, Avg Loss: {epoch_loss_avg.result():.4f}",
                    end="",
                )
        print(
            f"\r- Epoch {epoch+1}/{epochs} Complete. Final Avg Loss: {epoch_loss_avg.result():.4f}"
            + " " * 10
        )
...
def run_post_training_and_plot(
    exact_model_module,
    approx_model_module,
    post_train_data_x,
    post_train_data_y_onehot,
    ...
    if steps_per_epoch == 0:
        print(
            f"Error: Not enough post-training data ({len(post_train_data_x)} samples) for batch size {batch_size} to complete one epoch."
        )
        return
    ...
    if not hasattr(approx_model_module, "approx_learn"):
        print(
            f"ERROR: Approximate model does not have an 'approx_learn' method. Cannot proceed with its post-training."
        )
    ...
    for epoch in range(post_training_epochs):
        ...
        for step in range(steps_per_epoch):
            ...
            if run_approx_training:
                ...
                approx_loss_val_tensor = approx_model_module.approx_learn(
                    x_batch, y_batch
                )
            ...
            if step % 20 == 0:
                current_approx_loss_display = (
                    f"{approx_loss_val_np:.4f}"
                    if not np.isnan(approx_loss_val_np)
                    else "N/A"
                )
                print(
                    f"\r  Step {step}/{steps_per_epoch} | Exact Loss: {exact_loss_val_np:.4f}, Approx Loss: {current_approx_loss_display}",
                    end="",
                )
        ...
        if run_approx_training and epoch_approx_loss_metric.count.numpy() > 0:
            raw_approx_avg_losses_epoch.append(
                epoch_approx_loss_metric.result().numpy()
            )
            if actual_steps_in_epoch > 0:
                approx_avg_time_epoch.append(
                    epoch_approx_total_time / actual_steps_in_epoch
                )
            else:
        ...
        approx_avg_loss_display = (
            f"{epoch_approx_loss_metric.result():.4f}"
            if epoch_approx_loss_metric.count.numpy() > 0
            else "N/A"
        )
        print(
            f"\rPost-Training Epoch {epoch+1} Avg Losses -> Exact: {epoch_exact_loss_metric.result():.4f}, Approx: {approx_avg_loss_display}"
            + " " * 15
        )

    # Normalize losses
    first_epoch_exact_loss = (
        raw_exact_avg_losses_epoch[0]
        if len(raw_exact_avg_losses_epoch) > 0 and raw_exact_avg_losses_epoch[0] != 0
        else 1.0
    )
    first_epoch_approx_loss = (
        raw_approx_avg_losses_epoch[0]
        if len(raw_approx_avg_losses_epoch) > 0
        and not np.isnan(raw_approx_avg_losses_epoch[0])
        and raw_approx_avg_losses_epoch[0] != 0
        else 1.0
    )

    normalized_exact_losses = [
        loss / first_epoch_exact_loss for loss in raw_exact_avg_losses_epoch
    ]
    ...
    ax1.plot(
        ...
        marker="o",
        linestyle="-",
        color=color_exact_loss,
        label="Exact Model Norm. Loss",
    )
    ...
    if np.any(valid_approx_indices):
        ax1.plot(
            epochs_axis[valid_approx_indices],
            np.array(normalized_approx_losses)[valid_approx_indices],
            marker="x",
            linestyle="-",
            color=color_approx_loss,
            label="Approx. Model Norm. Loss",
        )
    ...
    ax2.plot(
        ...
        marker=".",
        linestyle="--",
        color=color_exact_time,
        label="Exact Model Avg. Epoch Time",
    )

    color_approx_time = "darkred"
    valid_approx_time_indices = ~np.isnan(
        approx_avg_time_epoch
    )  # Re-check for time, could be different if approx training failed early
    if run_approx_training and np.any(valid_approx_time_indices):
        ax2.plot(
            epochs_axis[valid_approx_time_indices],
            np.array(approx_avg_time_epoch)[valid_approx_time_indices],
            marker="+",
            linestyle="--",
            color=color_approx_time,
            label="Approx. Model Avg. Epoch Time",
        )
    ...
    plt.show()
    print(
        "Plot for Insight 2 (post-training normalized loss and time) saved as  ",
        IMG_DIR,
        "plot_post_training_normalized_loss_time_insight2.png",
    )
...
def run_insight2_experiment():
    ...
    exact_module_path_insight2 = OUT_DIR + "mnist_exact_for_posttraining_insight2"
    try:
        ...
        print(
            f"Exact model for Insight 2 not found (Error: {e}). Training for 1 epoch..."
        )
        exact_model_to_post_train = create_mnist_module(BATCH_SIZE)
        exact_model_to_post_train = initial_train_model(
            exact_model_to_post_train, (x_train, y_train, y_train_onehot), epochs=1
        )
        tf.saved_model.save(exact_model_to_post_train, exact_module_path_insight2)
        print(
            f"Exact model pre-trained for 1 epoch and saved to {exact_module_path_insight2}"
        )
    ...
    approx_kernel_base_structure = get_approx_kernel(
        FEATURES_SHAPE, OUTPUT_SHAPE, BATCH_SIZE
    )
    ...
    if hasattr(func_sub_for_approx_pretrain, "compile_exact"):
        print(
            "Calling compile_exact() on FuncSubstitute instance for approx pre-training setup..."
        )
    ...
    print(
        f"Pre-training approximation kernel with {num_samples_approx_pretrain} samples for {epochs_approx_pretrain} epochs via FuncSubstitute..."
    )
    func_sub_for_approx_pretrain.train_approx(
        use_provided=True,
    ...
    if not hasattr(approx_model_to_post_train, "approx_learn"):
        print(
            "ERROR: The pre-trained approximate kernel from FuncSubstitute does not have an 'approx_learn' method."
        )
        print(
            "       Cannot proceed with post-training comparison for Insight 2 as specified."
        )
        return
    ...
    if post_train_data_start_idx + post_train_data_count > len(x_train):
        ...
        if post_train_data_count < BATCH_SIZE:
            ...
            post_train_data_count = 10000
            if post_train_data_count > len(x_train):
                post_train_data_count = len(x_train)
            print(
                f"Warning: Adjusted post-training data to start from index 0 with {post_train_data_count} samples."
            )

    if post_train_data_count < BATCH_SIZE:
        print(
            f"Error: Insufficient data for post-training phase (need at least {BATCH_SIZE}, have {post_train_data_count})."
        )
        return

    post_train_x_subset = x_train[
        post_train_data_start_idx : post_train_data_start_idx + post_train_data_count
    ]
    post_train_y_subset_onehot = y_train_onehot[
        post_train_data_start_idx : post_train_data_start_idx + post_train_data_count
    ]
    ...
    run_post_training_and_plot(
        exact_model_module=exact_model_to_post_train,
